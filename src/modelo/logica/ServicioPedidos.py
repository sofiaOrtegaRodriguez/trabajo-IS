...
class ServicioPedidos:
    def crearPedido(self, sesion, items, total):
        """Crea un nuevo pedido en el Dao y devuelve el resultado."""
        return PedidoDaoJDBC().crear(sesion, items, total)

    def listarPedidos(self, sesion):
        """Devuelve la lista de pedidos del usuario desde el Dao."""
        return PedidoDaoJDBC().listar(sesion)

    def listarTodosPedidos(self):
        """Devuelve la lista de los pedidos del día desde el Dao."""
    ...
    def prepararPedidosVista(self, pedidos, estado="TODOS"):
        ...
        estado = str(estado).strip().upper()
        if not pedidos_filtrados: #Si no hay pedidos, prepara un mensaje de error
    ...
    def _estados_con_actual(self, estado_actual):
        """Comprueba que el estado actual esté en los estados permitidos, si no está lo añade"""
    ...
    def _estilo_estado_pedido(self, estado):
        """Establece el estilo del estado del pedido"""
