...
class ConexionSQLServer:
    ...
    jar_file = os.getenv(
        "SUSHULE_JDBC_JAR",
        str(Path(__file__).resolve().parents[3] / "lib" / "mssql-jdbc-13.4.0.jre11.jar"),
    )
    ...
    def createConnection(self):
        try:
            ...
            try:
                raw_connection.jconn.setAutoCommit(False)
            except Exception:
                try:
                    raw_connection.setAutoCommit(False)
                except Exception:
                    pass
    ...
    def getCursor(self):
        ...
        if self.conexion is None:
            raise RuntimeError(
                "No se pudo establecer la conexión JDBC con SQL Server. "
                "Verifica el driver JDBC, la ruta del JAR y la configuración de la base de datos."
            )
