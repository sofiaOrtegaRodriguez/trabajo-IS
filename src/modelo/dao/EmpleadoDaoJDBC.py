...
class EmpleadoDaoJDBC(EmpleadoDao, ConexionSQLServer):
    SQL_CHECK_LOGIN = """
    ...
    def consultarLogin(self, login_vo):
        ...
        try:
            ...
            rol = "administrador" if row.Emp_Tipo == "ADMINISTRADOR" else "gerente" if row.Emp_Tipo == "GERENTE" else "cajero"
    ...
    def actualizar(self, id_empleado, ssn, usuario, correo, contrasena, tipo):
        ...
        try:
            ...
            cursor.execute(self.SQL_UPDATE, (ssn, usuario, correo, contrasena, tipo, int(id_empleado)))
            if tipo == "CAJERO":
