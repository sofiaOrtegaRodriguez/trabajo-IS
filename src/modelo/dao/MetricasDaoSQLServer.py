...
class MetricasDaoSQLServer:
    ...
    def _fetch_resumen(self, cursor, inicio, fin):
        ...
        return {
            ...
            "ingresos_clientes": round(float(getattr(row, "ingresos_clientes", 0) or 0), 2),
        }
    ...
    def _fetch_productos_por_categoria(self, cursor, inicio, fin, category_column):
        ...
        cursor.execute(
            f"""
            SELECT
                LTRIM(RTRIM({self._quote_identifier(category_column)})) AS Categoria,
                Nombre
            FROM PRODUCTOS
        ...
        for row in cursor.fetchall():
            ...
            catalog.setdefault(categoria, []).append(row.Nombre)
        ...
        otros = [
            categoria
            for categoria in sorted(catalog.keys())
            if categoria not in {"Sushi", "Fritos", "Postres", "Bebidas"}
        ]
        for categoria in otros:
            productos = catalog.get(categoria, [])
            items = [
                {
                    "nombre": nombre,
            ...
            result.append(
                {
                    "anio": int(row.anio),
                    "mes": int(row.mes),
                    "label": f"{int(row.mes):02d}/{int(row.anio)}",
