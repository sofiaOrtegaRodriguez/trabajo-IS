...
from src.modelo.vo.PedidoVo import PedidoDetalleVo, PedidoTiempoRealVo, PedidoVo


class PedidoDaoJDBC(PedidoDao, ConexionSQLServer):
    ...
    def listarTiempoReal(self):
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                if pedido is None:
                    hora_value = row.Hora
                    usuario = row.IDCli if row.IDCli is not None else "Cajero"
                    if hasattr(hora_value, "strftime"):
                        fecha_text = hora_value.strftime("%d/%m/%Y")
                        hora_text = hora_value.strftime("%H:%M")
                    else:
                        fecha_text = str(hora_value)
                        hora_text = ""
                    pedido = PedidoTiempoRealVo(row.IDPed, fecha_text, hora_text, usuario, row.Estado, [], float(row.PrecioTotal))
                    pedidos[row.IDPed] = pedido
                if row.NombreProd is not None:
                    qty = int(row.Cantidad or 0)
